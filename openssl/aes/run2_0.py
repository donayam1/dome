...
def read_to_csv(base_dir,round):
    app="aes"
    # Set the directory containing the text files
    output_full_path=os.path.join(f"{base_dir}",f"{app}",f"round{round}","output.csv") 
    input_dir= os.path.join(f"{base_dir}",f"{app}",f"round{round}","output")
...
def run_experiment(round,base_dir):
    # round = args.round 
    app = "aes"
    input_csv = os.path.join(base_dir,f"{app}",f"round{round}","input.csv")
    output_dir = os.path.join(base_dir,f"{app}",f"round{round}","output")
    input_dir = os.path.join(f"{base_dir}",f"{app}","input")
    ...
    read_to_csv(round=round,base_dir=base_dir)

    print("Finished running experiment")

if __name__ == "__main__":
    # Argument parser setup
    parser = argparse.ArgumentParser(description="Process AES files from CSV input.")
    parser.add_argument("--round", type=str, default="round1", help="Specify the round number (default: round1)")
    parser.add_argument("--base_dir", type=str, help="Path to the input CSV file", required=True)
    args = parser.parse_args()

    run_experiment(args.round,args.base_dir)
