...
static void get_ecdsa_key_new(gcry_sexp_t *pkey, gcry_sexp_t *skey, int transient_key)
{
    gcry_sexp_t key_spec, key, pub_key, sec_key;
    int rc;

    // ECDSA key generation example:
    // NIST P-256 curve, with (flags dsa) to indicate ECDSA usage.
    rc = gcry_sexp_new(
        &key_spec,
        transient_key
            ? "(genkey (ecc (curve \"NIST P-256\")  (transient-key)))"
            : "(genkey (ecc (curve \"NIST P-256\") ))",
        0, 1);
    if (rc)
    ...
    if (rc)
    {
        ...
        {
            if (verbose)
                fprintf(stderr, "ECDSA keys are not available in FIPS mode.\n");
            return;
        }
        die("error generating ECDSA key: ", gcry_strerror(rc));
    }

    if (verbose > 1)
        show_sexp("generated ECDSA key:\n", key);
...
// For ECDSA: (data (flags raw) (value mpi)) where mpi is the hash.
// This is the same input format as DSA in raw mode.
vector<unsigned char> sign_data(gcry_sexp_t privkey, const unsigned char *data, size_t data_length)
{
    ...
    // The signature for ECDSA will look like: (sig-val (ecdsa (r r-mpi)(s s-mpi)))
