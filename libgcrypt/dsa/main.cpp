...
gcry_sexp_t import_key(const string &filename)
{
    ...
    gcry_sexp_t key;
    gcry_error_t err = gcry_sexp_new(&key, buffer.data(), buffer.size(), 1);
    if (err)
    {
        cerr << "Failed to create key S-expression: " << gcry_strerror(err) << endl;
        exit(1);
    }
    return key;
}
...
// Function to sign data using DSA
vector<unsigned char> dsa_sign(const gcry_sexp_t privkey, const unsigned char *data, size_t data_length, const char *events[], ofstream &file)
{
    ...
    if (err)
        handleErrors("Failed to create MPI from hash", err);

    gcry_sexp_t data_sexp;
    err = gcry_sexp_build(&data_sexp, NULL, "(data (flags raw) (value %m))", mpi_hash);
    gcry_mpi_release(mpi_hash);
    if (err)
        handleErrors("Failed to build data S-expression for signing", err);
    ...
    if (err)
        handleErrors("Signing failed", err);
...
bool verify_signature(const gcry_sexp_t pubkey, const unsigned char *data, size_t data_length, const vector<unsigned char> &signature)
{
    vector<unsigned char> digest = hash_data(data, data_length);
...
int main(int argc, char **argv)
{
    ...
    // Import keys
    gcry_sexp_t privkey = import_key(private_key_file_name);
    gcry_sexp_t pubkey = import_key(public_key_file_name);
    ...
    for (int i = 0; i < size - 2; i += 3)
    {
        ...
        vector<unsigned char> signature = dsa_sign(privkey, message, strlen((char *)message), events.data(), file);
