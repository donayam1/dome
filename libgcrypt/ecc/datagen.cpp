...
gcry_sexp_t generate_ecc_keypair()
{
    gcry_error_t err;
    gcry_sexp_t params;
    gcry_sexp_t keypair;

    // Build S-expression for key generation parameters (using NIST P-256 curve)
    err = gcry_sexp_build(&params, NULL, "(genkey (ecc (curve \"NIST P-256\")))");
    if (err)
    {
        std::cerr << "Failed to build key generation parameters: " << gcry_strerror(err) << std::endl;
        exit(1);
    }
    ...
    if (err)
    {
        std::cerr << "Failed to generate ECC key pair: " << gcry_strerror(err) << std::endl;
        exit(1);
    }
...
gcry_sexp_t encrypt_data(gcry_sexp_t pubkey, const unsigned char* data, size_t data_length)
{
    gcry_error_t err;
    ...
    err = gcry_sexp_build(&data_sexp, NULL, "(data (flags raw) (value %b))", data_length, data);
    if (err)
    {
...
void encrypt_with_multiple_keys(const unsigned char* input_data, size_t input_length, int n, const std::string& directory)
{
    for (int i = 1; i <= n; ++i)
    {
        ...
        gcry_sexp_t keypair = generate_ecc_keypair();
        ...
        gcry_sexp_t ciphertext = encrypt_data(pubkey, input_data, input_length);
